...
Application.init = function(opts) {
  ...
  this.event = new EventEmitter();  // event object to sub/pub events//事件对象用于发布和订阅事件
  ...
  appUtil.defaultConfiguration(this);//加载系统默认配置(包括启动命令行参数)

  this.state = STATE_INITED;//对象状态  初始化完毕
...
Application.load = function(name, component, opts) {
  ...
  if(typeof component === 'function') {//组件是工厂函数
    component = component(this, opts);//创建组件对象
  }

  if(!name && typeof component.name === 'string') {//组件自带名字
    name = component.name;
  }

  if(name && this.components[name]) {//组件已经存在 直接忽略
 ...
 * @memberOf Application
 */
//加载json的配置文件到settings中(加载基础配置文件，通过传入文件key 以及文件路径加载)
Application.loadConfigBaseApp = function (key, val, reload) {
  var self = this;
  var env = this.get(Constants.RESERVED.ENV);//获取当前的开发环境
  ...
  if(!!realPath && !!reload) {
    fs.watch(realPath, function (event, filename) {//监听文件变化，文件变化后重新加载配置文件
      if(event === 'change') {
 ...
 * //启动应用。将会导入默认的组件并且启动所有已经导入的组件
 * @param  {Function} cb callback function
 * @memberOf Application
 */
 Application.start = function(cb) {//启动成功后可以执行相关回调
  this.startTime = Date.now();//设置应用的启动时间
  if(this.state > STATE_INITED) {//标志应用已经在启动
  ...
  appUtil.startByType(self, function() {
    // 服务器进程启动成功
    //加载默认组件(创建不同服务器的默认组件对象)
    appUtil.loadDefaultComponents(self);
    
    var startUp = function() {
      //loaded:已经加载的组件
      appUtil.optComponents(self.loaded, Constants.RESERVED.START, function(err) {
        self.state = STATE_START;//应用状态切换到启动状态
        if(err) {
...
Application.afterStart = function(cb) {
  ...
  appUtil.optComponents(this.loaded, Constants.RESERVED.AFTER_START, function(err) {
    self.state = STATE_STARTED;//状态切换为启动完成
    var id = self.getServerId();
    if(!err) {
      logger.info('%j finish start', id);
    }
    if(!!afterFun) {//执行启动完成之后的生命周期钩子函数
    ...
    logger.info('%j startup in %s ms', id, usedTime); //记录统计启动时间
    self.event.emit(events.START_SERVER, id); //抛送时间给业务层，业务层获取到应用上的该时间进行逻辑处理操作
  });
};
 ...
 *app的处理，处理包括条件，如果所有条件都满足，处理器将会被执行。并且你可以设置多次尝试
 * 执行处理器。 处理log放在文件log或者处理log
 * @param {String} name transaction name
 * @param {Object} conditions functions which are called before transaction 执行函数前调用的条件检测函数
