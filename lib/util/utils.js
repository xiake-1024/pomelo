...
utils.loadCluster = function(app, server, serverMap) {
  ...
  for(var key in server) {
    ...
    if(value.indexOf(Constants.RESERVED.CLUSTER_SIGNAL) > 0) {//会更具集群服务器的自增长id，进行自增长的字段
  ...
  for(var i=0, l=seq; i<count; i++,l++) {
    var cserver = clone(server);//拷贝server对象
    //对集群的server进行命名 (cluster-server-server_type-num)
    ...
    for(var k in increaseFields) {
      ...
      cserver[k] = v + i;//根据集群服务器自增长的字段
    }
    serverMap[cserver.id] = cserver;//将字段进行集群修改证指定服务器类型的的服务器列表赋值给serverMap
  }
};
