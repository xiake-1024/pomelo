 ...
 * auto loaded filters
 */
//将模块绑定在pomelo对象上面pomelo.filters.name=各种过滤器对象
Pomelo.filters = {};
 ...
 * 自动绑定指定目录的所有组件
 *  比如: pomelo.master(文件名字)= master.js
 */
//读取componets下面的所有文件
fs.readdirSync(__dirname + '/components').forEach(function (filename) {
  if (!/\.js$/.test(filename)) {//确定是文件
    return;
  }
  var name = path.basename(filename, '.js');//获取文件名字
