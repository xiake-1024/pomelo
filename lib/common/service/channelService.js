...
/**
 * channle维护接受者集合，可以将消息发送给该频道并通过广播发送给接受者。
...
Channel.prototype.add = function(uid, sid) {
  if(this.state > ST_INITED) {
    ...
    var res = add(uid, sid, this.groups);//添加sid==>uid
    if(res) {//添加成功
...
var sendMessageByGroup = function(channelService, route, msg, groups, opts, cb) {
  ...
  var latch = countDownLatch.createCountDownLatch(count, function() {
    ...
    utils.invokeCallback(cb, null, failIds);//至少有一个执行fail或者全部执行成功，但是没有一个报错
  });
  ...
  var rpcCB = function(serverId) {
    return function(err, fails) {
      ...
      successFlag = true;//为true 代表至少有一个执行未err
      latch.done();
    };
  };
  ...
  var sendMessage = function(sid) {//更具server_sid发送消息
    return (function() {
      if(sid === app.serverId) {
