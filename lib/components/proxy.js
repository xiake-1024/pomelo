...
var genRpcClient = function(app, opts) {
  ...
  opts.routeContext = app;
  if(!!opts.rpcClient) {
    return opts.rpcClient.create(opts);//用户自定义创建方法
  } else {
    return Client.create(opts);//默认rpc创建方法
  }
};
 ...
 * 为rpc客户端获取代理路径,迭代所有的sys和user的remote服务器路径，创建remove路径的记录
 *  [{namespace,servertype,path_record}]
 * @param {Object} app current application context
...
//更具uid随机对指定servertype取余数，得到指定服务器的id
